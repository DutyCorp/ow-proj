$(document).ready(function() {
	...
	$('#btnSubmitPersonalClaimReport').click(function() {
		var EmployeeID = $('#selectClaimEmployee').val();
		...
		$.ajax({
			type: 'POST',
			url: '/GetPersonalClaimReportData',
			data: ClaimData,
			dataType: 'json',
			success:function(data){
				$('#tablePersonalClaimReport').html(data);
				$('#tablePersonalClaimReport').show();
				$('#tablePersonalClaimReport').css("width", "50%");
				$('#tablePersonalClaimReport').css("margin", "0 auto");
				$('#personalClaimReportTable').DataTable({
					dom: 'Bfrtip',
					lengthMenu: [
			            [ 10, 25, 50, -1 ],
					...
					buttons: [{extend:'pageLength',text: '<span>Show Page</span>'},	 {
			            ...
			            customize: function (xlsx) {
			                ...
			                //insertFileDate
			                var r1 = Addrow(1, [{ key: 'A', value: 'Personal Claim Report' }]);
